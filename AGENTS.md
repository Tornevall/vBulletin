...
- The signed redirect assertion must contain only the stable forum `userid`, state, audience, timestamps and nonce. Do not include username or email; Tools resolves those from its trusted direct forum integration after signature verification.
- Keep the callback as a top-level HTTPS GET redirect so Tools can validate the initiating browser session under its SameSite=Lax session policy. Do not change the bridge to a cross-site POST without a replacement session-binding design.
